...
bool ModuleFileSystem::Init(ConfigEditor* config)
{
	LOG("Loading File System");
	bool ret = true;

	// Ask SDL for a write dir
	char* write_path = SDL_GetPrefPath(App->organization_name.c_str(), App->app_name.c_str());
...
{
	//Assets	
	if (!Exists("Assets/Meshes/")) {
		CreateDirectory("Assets/Meshes/");
		LOG("Creating directory %s", "Assets/Meshes/");
	}

	if (!Exists("Assets/Textures/")) {
		CreateDirectory("Assets/Textures/");
		LOG("Creating directory %s", "Assets/Textures/");
	}

	if (!Exists("Assets/Scenes/")) {
		CreateDirectory("Assets/Scenes/");
		LOG("Creating directory %s", "Assets/Scenes/");
	}

	if (!Exists("Assets/Scripts/")) {
		CreateDirectory("Assets/Scripts/");
		LOG("Creating directory %s", "Assets/Scripts/");
	}

	//Library
	if (!Exists("Library/Meshes/")) {
		CreateDirectory("Library/Meshes/");
		LOG("Creating directory %s", "Library/Meshes/");
	}

	if (!Exists("Library/Scenes/")) {
		CreateDirectory("Library/Scenes/");
		LOG("Creating directory %s", "Library/Scenes/");
	}
